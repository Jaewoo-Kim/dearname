...
function calcSajuOhengGrade(hanjaOhengs, birthDate, birthTime, options = {}) {
  ...
  if (!weakOhengs.length)
    return { grade:'매우 좋음', desc:'사주 오행이 고루 균형 잡혀 있어 특별한 보완이 필요 없습니다. 이름의 자원오행이 사주와 잘 어우러집니다.' };
  ...
  return   { grade:'매우 나쁨', desc:`사주에 부족한 [${weakOhengs.join(', ')}] 기운이 이름에 반영되지 않았습니다. 해당 오행의 한자 선택을 권장합니다.` };
}
