 ...
 */
function calcSaju(dateStr, timeStr) {
  ...
  return {
    ...
    month: getMonthPillar(y, m, d, h, min || 0),
    day:   dayPillar,
    time:  getTimePillar(dayPillar.gan, h)
  };
}
 ...
 */
function calcSajuOhengGrade(hanjaOhengs, birthDate, birthTime) {
  if (!birthDate) {
    return {
      grade: '평',
  ...
  const saju = calcSaju(birthDate, birthTime || '12:00');
  if (!saju) {
