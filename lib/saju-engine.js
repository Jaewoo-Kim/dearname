...
// 종격(從格) 판정용 — name-spec.js의 CONTROLLERS/SUPPORTERS와 동일 값
...
function calcSajuOhengGrade(hanjaOhengs, birthDate, birthTime, options = {}) {
  ...
  // ── 종격 체크 (지장간 가중 점수 기준 — buildNameSpec과 동일 조건) ──────
  const _wsc  = calcOhengScores(saju);
  const _KR3  = {'木':'나무(木)','火':'불(火)','土':'흙(土)','金':'쇠(金)','水':'물(水)'};
  for (const [_jOheng, _jScore] of Object.entries(_wsc)) {
    if (_jScore < 60) continue;
    const _jCtrl = _JONGGYEOK_CTRL[_jOheng];
    if (_wsc[_jCtrl] >= 5) continue;                   // 극오행이 5점 이상 → 종격 아님
    // 종격 확정
    const _jSup     = _JONGGYEOK_SUP[_jOheng];
    const _hitsCtrl = hanjaOhengs.includes(_jCtrl);
    const _hitsDom  = hanjaOhengs.includes(_jOheng);
    const _hitsSup  = hanjaOhengs.includes(_jSup);
    if (_hitsCtrl)
      return { grade:'나쁨',      desc:`이 사주는 ${_KR3[_jOheng]} 기운의 종격입니다. 이름이 ${_KR3[_jOheng]} 기운을 억제하는 ${_KR3[_jCtrl]} 기운을 포함해 좋지 않습니다.` };
    if (_hitsDom || _hitsSup)
      return { grade:'매우 좋음', desc:`이 사주는 ${_KR3[_jOheng]} 기운의 종격입니다. 이름이 ${_KR3[_jOheng]} 기운을 강화하여 매우 좋습니다.` };
    return   { grade:'좋음',      desc:`이 사주는 ${_KR3[_jOheng]} 기운의 종격입니다. ${_KR3[_jOheng]} 또는 ${_KR3[_jSup]} 기운의 한자를 선택하면 더욱 좋습니다.` };
  }

  // ── 통관(通關) 체크 (지장간 가중 점수 기준 — buildNameSpec과 동일 조건) ──
  // 상극 쌍이 각 25점 이상이면 중재 오행(통관 용신)이 불균형 보완보다 우선
  const _TG_PAIRS = [['木','土'],['火','金'],['土','水'],['金','木'],['水','火']];
  const _TG_MED   = {
    '木土':'火','土木':'火','火金':'土','金火':'土',
    '土水':'金','水土':'金','金木':'水','木金':'水','水火':'木','火水':'木'
  };
  for (const [_tA, _tB] of _TG_PAIRS) {
    if (_wsc[_tA] < 25 || _wsc[_tB] < 25) continue;
    const _tMed = _TG_MED[_tA + _tB];
    if (!_tMed) continue;
    const _hitsMed   = hanjaOhengs.includes(_tMed);
    const _hitsClash = hanjaOhengs.some(o => o === _tA || o === _tB);
    if (_hitsMed) {
      return { grade:'매우 좋음', desc:`이 사주는 ${_KR3[_tA]}과 ${_KR3[_tB]}이 강하게 대립하는 통관 사주입니다. 이름의 ${_KR3[_tMed]} 기운이 두 기운을 중재하여 매우 좋습니다.` };
    }
    if (_hitsClash) {
      const _clashInName = [_tA, _tB].filter(o => hanjaOhengs.includes(o)).map(o => _KR3[o]).join(', ');
      return { grade:'나쁨', desc:`이 사주는 ${_KR3[_tA]}과 ${_KR3[_tB]}이 강하게 대립하는 통관 사주입니다. 이름의 ${_clashInName} 기운이 대립을 심화시킵니다. 중재 기운인 ${_KR3[_tMed]}의 한자 선택을 권장합니다.` };
    }
    return { grade:'좋음', desc:`이 사주는 ${_KR3[_tA]}과 ${_KR3[_tB]}이 강하게 대립하는 통관 사주입니다. ${_KR3[_tMed]} 기운의 한자를 선택하면 두 기운을 중재하여 더욱 좋습니다.` };
  }
