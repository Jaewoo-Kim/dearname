...
function calcSajuOhengGrade(hanjaOhengs, birthDate, birthTime, options = {}) {
  ...
  if (!weakOhengs.length) {
    ...
    if (!relWeak.length || !hanjaOhengs.length) {
      return { grade:'매우 좋음', desc:'사주 오행이 완전히 균등하게 분포되어 있습니다. 이름의 자원오행과 자연스럽게 조화를 이룹니다.' };
    }
    ...
    if (hitsRelWeak && !hitsRelStrong) {
      return { grade:'매우 좋음', desc:`균형 잡힌 사주에서 상대적으로 약한 ${weakInName} 기운을 이름이 보충합니다. 사주의 부족한 기운을 이름으로 채운 탁월한 선택입니다.` };
    }
    if (hitsRelWeak && hitsRelStrong) {
  ...
  if (compensated.length === weakOhengs.length)
    return { grade:'매우 좋음', desc:`사주에 부족한 ${weakOhengs.join(', ')} 기운을 선택하신 한자가 완벽히 보완합니다. 탁월한 선택입니다.` };
  if (compensated.length > 0)
    return { grade:'좋음',     desc:`${compensated.join(', ')}는 보완됩니다. ${uncompensated.join(', ')} 기운 보완도 고려해보세요.` };
  return   { grade:'나쁨',     desc:`사주에 없는 ${weakOhengs.join(', ')} 기운이 이름에 반영되지 않았습니다. 해당 오행의 한자 선택을 권장합니다.` };
}
