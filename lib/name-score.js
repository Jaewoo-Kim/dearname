...
var NameScore = (function () {
  ...
  function _choOheng(kr) {
    if (!kr) return null;
    return CHO_OHENG[_getCho(kr)] || null;
  }
  // 획수 끝자리 → 오행 (수리오행)
  function _numOheng(n) {
    const d = n % 10;
    if (d === 1 || d === 2) return '木';
    if (d === 3 || d === 4) return '火';
    if (d === 5 || d === 6) return '土';
  ...
  // 비화(같은 오행)도 막힘이 없는 것으로 본다 — 보고서 판정과 동일 기준
  function _isSangsaeng(a, b) {
    if (!a || !b) return true;
    if (a === b) return true;
    const ia = OHENG_CYCLE.indexOf(a), ib = OHENG_CYCLE.indexOf(b);
    return (ia + 1) % 5 === ib || (ib + 1) % 5 === ia;
  }

  /**
   * 구조 지표 원시값 — 사주에 의존하지 않는 다섯 지표의 계산 근거를 한곳에서 만든다.
   * 채점(scoreCombo)과 보고서 판정(gradeStructural)이 같은 값을 쓰게 해
   * "점수는 괜찮다는데 라벨은 나쁘다"는 어긋남이 생기지 않도록 한다.
   */
  function _structuralMetrics(combo, familyName, SURI_DATA) {
    ...
    const nums = [g1, g2, g3, g4];

    const _sg = (n) => { let v = n > 81 ? n % 81 : n; if (v === 0) v = 81; return SURI_DATA?.[v]?.grade || '평'; };
    const suriGrades = nums.map(_sg);

    // 수리오행 — 4격 인접 쌍(원↔형, 형↔이, 이↔정)의 상극 개수 0~3
    const moOhengs = nums.map(_numOheng);
    let moBad = 0;
    for (let i = 0; i < moOhengs.length - 1; i++) {
      if (!_isSangsaeng(moOhengs[i], moOhengs[i + 1])) moBad++;
    }

    // 수리음양 — 각 한자 획수의 홀짝이 한쪽으로만 쏠렸는지
    const strokes = [s0, s1, s2].filter(Boolean);
    const yinYangTags = strokes.map(s => s % 2 === 0 ? '음' : '양');
    const suriYinAllSame = strokes.length > 1 && yinYangTags.every(t => t === yinYangTags[0]);

    // 발음오행 — 인접 음절 간 상극 개수
    const chars = [familyName.kr, h1.kr, h2?.kr].filter(Boolean);
    const pronOhengs = chars.map(_choOheng);
    let pronBad = 0, pronPairs = 0;
    for (let i = 0; i < pronOhengs.length - 1; i++) {
      const a = pronOhengs[i], b = pronOhengs[i + 1];
      if (!a || !b) continue;
      pronPairs++;
      if (!_isSangsaeng(a, b)) pronBad++;
    }

    // 발음음양 — 모음의 음양이 한쪽으로만 쏠렸는지
    const jungTags = chars.map(c => YANG_JUNG.has(_getJung(c)) ? '양' : '음');
    const pronYinAllSame = chars.length > 1 && jungTags.every(t => t === jungTags[0]);

    return { nums, suriGrades, moBad, suriYinAllSame, pronBad, pronPairs, pronYinAllSame };
  }

  /**
   * 구조 지표 5종 등급 판정 — 보고서 배지와 후보 정렬이 공유하는 단일 기준.
   * 자원오행(사주 보완)은 만세력이 필요해 여기 포함하지 않는다.
   * @returns {Object} { 사격수리, 발음오행, 수리오행, 발음음양, 수리음양 }
   */
  function gradeStructural(combo, familyName, deps) {
    deps = deps || {};
    const m = _structuralMetrics(combo, familyName, deps.SURI_DATA || null);

    const daegil = m.suriGrades.filter(g => g === '대길').length;
    const good   = m.suriGrades.filter(g => g === '대길' || g === '길').length;
    const bad    = m.suriGrades.filter(g => g === '흉' || g === '대흉').length;
    const daehyung = m.suriGrades.filter(g => g === '대흉').length;
    const 사격수리 = (daehyung > 0 || bad >= 3) ? '매우 나쁨'
                   : bad >= 2                   ? '나쁨'
                   : (bad === 1 && good >= 3)   ? '좋음'
                   : bad > 0                    ? '나쁨'
                   : daegil >= 2                ? '매우 좋음'
    ...
    // 4격 끝자리가 모두 상생으로 이어지기는 구조적으로 어려워, 2곳 상극까지는 '보통'으로 본다.
    const 수리오행 = m.moBad === 0 ? '매우 좋음'
                   : m.moBad === 1 ? '좋음'
                   : m.moBad === 2 ? '보통'
                   :                 '나쁨';

    const 발음오행 = m.pronPairs === 0        ? '보통'
                   : m.pronBad === 0          ? '매우 좋음'
                   : m.pronBad === m.pronPairs ? '매우 나쁨'
                   : m.pronBad * 2 <= m.pronPairs ? '좋음'
                   :                                '나쁨';

    // 음양은 균형/불균형 두 상태뿐이라 등급도 두 단계다. 다만 불균형을 '매우 나쁨'으로
    // 표기하면 실제 위험도에 비해 과하게 읽혀, 한 단계 낮춘 '나쁨'으로 표시한다.
    const 수리음양 = m.suriYinAllSame ? '나쁨' : '매우 좋음';
    const 발음음양 = m.pronYinAllSame ? '나쁨' : '매우 좋음';

    return { 사격수리, 발음오행, 수리오행, 발음음양, 수리음양 };
  }

  const _BAD_GRADES = new Set(['나쁨', '매우 나쁨']);
  /** 구조 지표 중 '나쁨' 이상이 하나라도 있으면 true — 후보 정렬에 쓴다. */
  function hasBadIndicator(combo, familyName, deps) {
    const g = gradeStructural(combo, familyName, deps);
    return Object.keys(g).some(k => _BAD_GRADES.has(g[k]));
  }
  ...
  function scoreCombo(combo, nameSpec, familyName, state, deps) {
    ...
    // [L] 수리오행 — 4격 끝자리 오행이 상생으로 이어지는지.
    // 보고서는 이 지표로 '나쁨'까지 표시하는데 채점에는 빠져 있어,
    // 점수는 높은데 배지는 나쁜 이름이 상위에 올라오던 원인이었다.
    const _moOhengs = [g1, g2, g3, g4].map(_numOheng);
    let _moBad = 0;
    for (let i = 0; i < _moOhengs.length - 1; i++) {
      if (!_isSangsaeng(_moOhengs[i], _moOhengs[i + 1])) _moBad++;
    }
    if (_moBad >= 3) return -999;                       // 전 구간 상극 → 탈락
    score += _moBad === 0 ? 15 : _moBad === 1 ? 5 : -25;
    ...
    for (let i = 0; i < seq.length - 1; i++) {
      ...
      else { score -= 10; _pronBad++; }
    }
    // 두 글자 이름에서 인접 음절이 모두 상극이면 소리의 흐름이 완전히 끊긴다 → 탈락.
    // 외자는 音 전이가 한 번뿐이라 이 조건이 곧 "상극 한 번"이 되어버리므로 제외한다.
    if (_pronPairs >= 2 && _pronBad === _pronPairs) return -999;

    // [D] 수리음양 — 획수 홀짝이 한쪽으로만 쏠리면 감점.
    // 하드 탈락시키면 외자처럼 글자 수가 적은 경우 후보의 절반이 사라져 감점으로 다룬다.
    const strokes = [s0,s1,s2].filter(Boolean);
    if (strokes.length > 1) {
      const tags = strokes.map(s => s%2===0?'음':'양');
      score += tags.every(t => t===tags[0]) ? -30 : 10;
    }

    // [E] 발음음양 — 모음 음양이 한쪽으로만 쏠리면 감점(위와 같은 이유)
    const chars = [familyName.kr, h1.kr, h2?.kr].filter(Boolean);
    if (chars.length > 1) {
      const yy = chars.map(c => { const j=_getJung(c); return YANG_JUNG.has(j)?'양':'음'; });
      score += yy.every(t => t===yy[0]) ? -30 : 5;
    }
  ...
  return { scoreCombo, gradeStructural, hasBadIndicator, BUNPA_HANJA, LUCK_SURI };
})();
