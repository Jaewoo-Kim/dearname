...
function _scoreCombo(combo, nameSpec, familyName, state) {
  ...
  if (grades.some(g => g === '대흉')) return -999;  // 대흉: 즉시 탈락
  for (const g of grades) {
    if (g === '대길')      score += 20;   // 대길: 프리미엄 보너스
    else if (g === '길')   score += 15;   // 길: +15 (기존 +5 → 대폭 상향)
    else if (g === '평')   score -= 5;    // 평: 소폭 패널티
    else if (g === '흉')   score -= 60;   // 흉: 사실상 탈락 수준 패널티
  }
  // 4격 전부 길 이상(길·대길)이면 올클 보너스
  if (grades.every(g => g === '길' || g === '대길')) score += 25;
  ...
  // 음수 허용: 흉 페널티로 인한 음수가 threshold 필터에 걸려야 탈락 처리됨
