...
// ── 분파(分破) 한자 목록 ───────────────────────────────────
// 좌우 또는 상하로 뚜렷하게 나뉘는 구조의 한자
// 이름의 모든 글자(성 포함)가 전부 분파 구조이면 기운이 흩어진다고 보아
// 하드 필터로 제외 (성명학 표준 분류 기준)
const BUNPA_HANJA = new Set([
  ...
  '伸','佑','伯','仲','仙','倡','俐','傑','傑','偲','佶',
  '儁','儷','僖','儼',
  // ── 좌우분파: 扌旁 (손 수 변) ──
  '振','承','揚','揮','摯','擇','拓','振','撥','摩',
  // ── 좌우분파: 土旁 (흙 토 변) ──
  '均','坦','坤','垣','培','堅','堯','堉','塤','墉','均',
  '埈','堰','埠',
  // ── 좌우분파: 火旁 (불 화 변) ──
  '炯','炳','煥','燦','煒','燁','燮','熙','炫',
  // ── 좌우분파: 糸旁 (실 사 변) ──
...
function _scoreCombo(combo, nameSpec, familyName, state) {
  ...
  // [분파 하드 필터] 성+이름 전체가 분파 구조 한자로만 구성되면 절대 탈락
  // (기운이 사방으로 흩어진다는 성명학 원칙 — 한 글자라도 非분파이면 통과)
  ...
  // ① 대흉은 어디든 절대 탈락 (유일한 하드필터)
  if (grades.some(g => g === '대흉')) return -999;

  // ② 말년(정격)은 길 이상 필수 — 평·흉이면 탈락
  if (grades[3] === '평' || grades[3] === '흉') return -999;

  // ③ 격별 차등 점수
  //    우선순위: 대길 >> 길 >> (초년은 평·흉 소폭 허용) >> (청년·장년 평·흉 큰 패널티)
  //    → 평·흉이 나와야 한다면 자동으로 초년에 몰리도록 설계
  const _suriScore = (g, pos) => {
    // pos: 0=초년, 1=청년, 2=장년, 3=말년
    if (g === '대길') return pos === 3 ? 35 : 25;  // 말년 대길에 추가 보너스
    if (g === '길')   return pos === 3 ? 20 : 15;
    if (g === '평')   return pos === 0 ? -15 : -45; // 초년은 소폭, 청년·장년은 큰 패널티
    if (g === '흉')   return pos === 0 ? -35 : -90; // 초년은 중간, 청년·장년은 매우 큰 패널티
    return 0;
  };
