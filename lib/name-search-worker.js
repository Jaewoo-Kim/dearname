...
function _scoreCombo(combo, nameSpec, familyName, state) {
  ...
  // grades[0]=원격(초년), grades[1]=형격(청년), grades[2]=이격(장년), grades[3]=정격(말년)
  const grades = [_sg(g1), _sg(g2), _sg(g3), _sg(g4)];

  // ① 말년(정격) 무조건 대길 — 위반 시 즉시 탈락
  if (grades[3] !== '대길') return -999;

  // ② 대흉은 어디든 즉시 탈락
  if (grades.some(g => g === '대흉')) return -999;

  // ③ 격별 가중 점수 — 평·흉은 초년(원격)에만 소폭 허용, 나머지 격은 사실상 차단
  const _suriScore = (g, isChonyeon) => {
    if (g === '대길') return 25;
    if (g === '길')   return 15;
    if (g === '평')   return isChonyeon ? -20 : -80;  // 초년은 소폭, 청년/장년은 사실상 탈락
    if (g === '흉')   return isChonyeon ? -50 : -999; // 초년은 중패널티, 청년/장년은 즉시탈락
    return 0;
  };

  const s_chonyeon  = _suriScore(grades[0], true);   // 원격(초년) — 평/흉 소폭 허용
  const s_cheongnyon = _suriScore(grades[1], false);  // 형격(청년)
  const s_jangnyeon  = _suriScore(grades[2], false);  // 이격(장년)
  const s_malnyeon   = 25;                             // 정격(말년) — 대길 보장됨

  if (s_chonyeon  === -999) return -999;
  if (s_cheongnyon === -999) return -999;
  if (s_jangnyeon  === -999) return -999;

  score += s_chonyeon + s_cheongnyon + s_jangnyeon + s_malnyeon;
