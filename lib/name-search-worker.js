...
function _scoreCombo(combo, nameSpec, familyName, state) {
  ...
  // ① 말년(정격, g4) 무조건 대길 — 위반 시 즉시 탈락
  if (grades[3] !== '대길') return -999;

  // ② 흉·대흉·평 → 즉시 탈락 (길 이상만 허용)
  if (grades.some(g => g === '대흉' || g === '흉' || g === '평')) return -999;

  // ③ 길/대길 가점 (최대 100점 + 올클 보너스 30점)
  for (const g of grades) {
    if (g === '대길') score += 25;
    else if (g === '길') score += 15;
  }
  if (grades.every(g => g === '대길')) score += 30; // 4격 전부 대길 올클 보너스
