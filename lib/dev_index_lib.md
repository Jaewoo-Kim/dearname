...
**사주 분류 & 등급 로직 (8자 단순 카운트, 지장간 미포함):**

| 사주 유형 | 조건 | 이름 행동별 등급 |
|---|---|---|
| 불균형 | 0개 오행 존재 | 부족 오행 전부 보완=매우 좋음 / 일부=좋음 / 미보완=나쁨 |
| 편중 | 0개 없고 최댓값 `=== 3` (`{3,2,1,1,1}`) | 쏠린(지배) 오행 강화 시 **나쁨** (약한 것 일부 채워도) / 약한 것만 보충=매우 좋음 |
| 균형 | 0개 없고 최댓값 `≤ 2` | relWeak만=매우 좋음 / 혼합=좋음 / relStrong만=나쁨 |

- `_isSkewed = _maxCnt === 3`, `_domSet` = 최댓값 오행. `_nameHitsDom`이면 편중형에서 나쁨 강등.
- index.html 셀프 진단 뱃지(`self-oheng-balance-badge`)·진단 멘트·안내 문구가 동일 분류와 동기화됨 (균형=초록/편중=주황/불균형=빨강).
