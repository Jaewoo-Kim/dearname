...
export default function MaintenanceSettingsForm({ initial }: { initial: MaintenanceSettings }) {
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-card">
      <div className="flex items-center justify-between gap-4">
        ...
        <button
          ...
          className={`relative h-6 w-11 shrink-0 rounded-full transition-colors ${enabled ? 'bg-red-500' : 'bg-slate-200'}`}
        >
          <span
      ...
      <textarea
        ...
        rows={2}
        className="mt-4 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500"
      />
