...
- 화면: 대시보드(월/연 전환 + 월·연 선택 + 매출 추이 12개월 고정·상품 비중·결제수단 그래프) /
  ...
  전환율 퍼널 / AI 비용 / 한자 DB(검색+수정+금기 한자 관리) /
  운영 관리(가격·프로모션·상품 구성·점검모드 + 서브탭으로 계정 관리·감사 로그)
...
이메일+비밀번호 방식이며 자가 가입은 불가능합니다 — 어드민만 `/settings`의 "계정 관리" 서브탭에서 새 계정(이메일·
...
- 최초 배포 시 `admin_users` 테이블이 비어있으면, 그 상태에서 처음 로그인하는 계정이 자동으로
  ...
  이메일+비밀번호로 직접 만들어야 합니다 — 이후 신규 팀원은 `/settings`의 "계정 관리" 서브탭에서 어드민이 만들면 됩니다.
- 등급: 어드민(등급 관리+운영 관리 즉시 반영+승인), 편집자(대부분의 쓰기 작업 가능, 운영
  관리 변경은 어드민 승인 필요), 뷰어(조회만).
- `/settings`의 "계정 관리" 서브탭에서 계정 추가·등급 변경·비밀번호 강제 재설정·계정 삭제(Supabase Auth 사용자까지 함께
