...
export default async function AuditLogsPage({
  ...
  return (
    <div>
      ...
      <PeriodFilterBar basePath="/settings/audit-logs" baseParams={baseParams} period={period} />

      <div className="mb-4">
        <form className="flex flex-wrap gap-2" action="/settings/audit-logs" method="get">
      ...
      <div className="mt-4 flex items-center justify-between text-sm text-slate-500">
        ...
        <div className="flex gap-3">
          {page > 1 && (
            <Link className="hover:underline" href={buildQueryHref('/settings/audit-logs', { ...baseParams, page: String(page - 1) })}>이전</Link>
          )}
          <span>{page} / {totalPages}</span>
          {page < totalPages && (
            <Link className="hover:underline" href={buildQueryHref('/settings/audit-logs', { ...baseParams, page: String(page + 1) })}>다음</Link>
          )}
        </div>
      </div>
    </div>
