...
function _genSuriStory(candidate) {
  ...
  const endWord = d4.grade === '대길' ? '더없이 풍요로운'
    : d4.grade === '길' ? '풍요로운'
    : d4.grade === '흉' ? '단단히 다져야 할'
    : d4.grade === '대흉' ? '각별히 유의해야 할'
    : '안정적인';
...
function _genLifeFlow(candidate) {
  ...
  return {
    ...
    late:   d4.grade === '대흉'
      ? `정격 ${g4}수 ${d4.name}은 대흉(大凶)에 해당하는 자리로, 평생의 총운에서 각별한 주의가 필요합니다. 다른 격국의 강점을 살리되, 이 결함을 보완할 대안을 함께 검토하시길 권합니다.`
      : d4.grade === '흉'
      ? `정격 ${g4}수 ${d4.name}에 다소 흉(凶)한 기운이 있어, 말년에는 꾸준한 노력과 관리가 필요합니다. 다만 다른 격국이 뒷받침되어 전체적인 균형은 무너지지 않습니다.`
      : `정격 ${g4}수 ${d4.name}이 수십 년의 노력을 아름답게 거두어들입니다. 이름이 품은 깊은 기운이 지혜와 인덕으로 빛나는 풍요로운 시기를 열어줍니다.`
  };
}
...
【공통 문체 규칙 — 모든 필드에 적용, 위반 시 다시 쓸 것】
- 줄표(—)로 삽입구나 부연 설명을 끼워 넣지 말 것. 문장을 끊어서 따로 쓸 것.
- 화살표(→)로 인과·변화를 잇지 말 것. "그 결과", "그래서", "~하게 됩니다" 같은 자연스러운 접속으로 풀어 쓸 것.
- 한 문장에 가운뎃점(·)으로 네 개 이상 나열하지 말 것. 세 개까지만 쓰고, 그 이상이면 쉼표로 풀거나 문장을 나눌 것.
- "단순히 ~가 아니라 ~입니다", "~뿐만 아니라 ~까지" 같은 대구 구문을 반복해서 쓰지 말 것.
- 실제로 사람이 편지·상담에서 말하듯 자연스러운 구어체 문장으로 쓸 것.
